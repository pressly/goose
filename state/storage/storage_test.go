...
// The goal of this test is to verify the storage package works as expected. This test is not
...
func TestStore(t *testing.T) {
	...
	t.Run("postgres", func(t *testing.T) {
		...
		check.NoError(t, err)
		t.Cleanup(cleanup)
		testStore(context.Background(), t, storage.PostgreSQLWithTableName("test_goose_db_version"), db, func(t *testing.T, err error) {
	...
	t.Run("sqlite3", func(t *testing.T) {
		...
		check.NoError(t, err)
		testStore(context.Background(), t, storage.Sqlite3WithTableName("test_goose_db_version"), db, func(t *testing.T, err error) {
...
func testStore(ctx context.Context, t *testing.T, store state.Storage, db *sql.DB, alreadyExists func(t *testing.T, err error)) {
	...
	for i := 0; i < 6; i++ {
		err = runConn(ctx, db, func(conn *sql.Conn) error {
			return store.InsertVersion(ctx, conn, int64(i))
		})
		check.NoError(t, err)
	}
	...
	err = runConn(ctx, db, func(conn *sql.Conn) error {
		...
		for i := 0; i < 6; i++ {
			check.Number(t, res[i].VersionID, 5-i)
		}
		return nil
	})
	check.NoError(t, err)
	...
	for i := 5; i >= 3; i-- {
		err = runConn(ctx, db, func(conn *sql.Conn) error {
			return store.DeleteVersion(ctx, conn, int64(i))
		})
		check.NoError(t, err)
	}
	...
	err = runConn(ctx, db, func(conn *sql.Conn) error {
		...
		for i := 0; i < 3; i++ {
			check.Number(t, res[i].VersionID, 2-i)
		}
		return nil
	})
	check.NoError(t, err)
	...
	// 1. *sql.Tx
	err = runTx(ctx, db, func(tx *sql.Tx) error {
		return store.DeleteVersion(ctx, tx, 2)
	})
	check.NoError(t, err)
	// 2. *sql.Conn
	err = runConn(ctx, db, func(conn *sql.Conn) error {
		return store.DeleteVersion(ctx, conn, 1)
	})
	check.NoError(t, err)
	// 3. *sql.DB
	err = store.DeleteVersion(ctx, db, 0)
	check.NoError(t, err)
