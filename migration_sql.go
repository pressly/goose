...
func runSQLMigration(
	...
	if useTx {
		...
		if !noVersioning {
			if direction {
				if err := getDialect().InsertVersion(ctx, tx, v); err != nil {
					verboseInfo("Rollback transaction")
					_ = tx.Rollback()
					return fmt.Errorf("failed to insert new goose version: %w", err)
				}
			} else {
				if err := getDialect().DeleteVersion(ctx, tx, v); err != nil {
					verboseInfo("Rollback transaction")
					_ = tx.Rollback()
	...
	if !noVersioning {
		if direction {
			if err := getDialect().InsertVersionNoTx(ctx, db, v); err != nil {
				return fmt.Errorf("failed to insert new goose version: %w", err)
			}
		} else {
			if err := getDialect().DeleteVersionNoTx(ctx, db, v); err != nil {
				return fmt.Errorf("failed to delete goose version: %w", err)
			}
		}
	}
