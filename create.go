...
func Create(
	...
	switch sourceType {
	case SourceTypeGo, SourceTypeSQL:
	default:
		return "", fmt.Errorf("invalid soure type: %v", sourceType)
	}
	now := time.Now()
	if opt.timeFunc != nil {
		now = opt.timeFunc()
	...
	if opt.Sequential {
		...
		vMigrations, err := versioned(sources)
		if err != nil {
			return "", err
		}
		var v int64 = 1
		if len(vMigrations) > 0 {
			v = vMigrations[len(vMigrations)-1] + 1
		}
		version = fmt.Sprintf(seqVersionFormat, v)
	}
	filename := fmt.Sprintf("%v_%v.%v", version, snakeCase(name), sourceType.String())
...
func versioned(in []*Source) ([]int64, error) {
	var versions []int64
