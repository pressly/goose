...
func (p *Provider) maybeInsertOrDelete(
	...
	if direction {
		return p.store.InsertVersion(ctx, db, migration.NewVersion(version))
	}
	return p.store.DeleteVersion(ctx, db, migration.NewVersion(version))
}
...
func (p *Provider) tryEnsureVersionTable(ctx context.Context, conn *sql.Conn) error {
	...
	return retry.Do(ctx, b, func(ctx context.Context) error {
		exists, err := p.store.TableVersionExists(ctx, conn)
		if err == nil && exists {
			return nil
			...
			if res, err := p.store.GetMigration(ctx, conn, migration.ZeroVersion); err == nil && res != nil {
				return nil
			}
		...
		if err := beginTx(ctx, conn, func(tx *sql.Tx) error {
			...
			return p.store.InsertVersion(ctx, tx, migration.ZeroVersion)
		}); err != nil {
...
func (p *Provider) runMigration(ctx context.Context, db DBTxConn, m *Migration, direction bool) error {
	switch m.Type {
	case TypeGo:
...
func (p *Provider) runGo(ctx context.Context, db DBTxConn, m *Migration, direction bool) (retErr error) {
	defer func() {
		if r := recover(); r != nil {
...
func (p *Provider) runSQL(ctx context.Context, db DBTxConn, m *Migration, direction bool) error {
