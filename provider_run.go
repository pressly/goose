...
func (p *Provider) tryEnsureVersionTable(ctx context.Context, conn *sql.Conn) error {
	...
	return retry.Do(ctx, b, func(ctx context.Context) error {
		...
		if err == nil && exists {
			...
			//
			// When direct table existence checks aren't supported, we attempt to query the
			// sentinel migration. This approach has two implications:
			//
		...
		if p.cfg.isolateDDL {
			...
			if err := p.store.Insert(ctx, conn, database.InsertRequest{Version: sentinel}); err != nil {
				return retry.RetryableError(fmt.Errorf("insert sentinel version: %w", err))
			}
		} else {
			// If DDL isolation is not enabled, we can create the version table and insert the
			// sentinel version in a single transaction.
			if err := beginTx(ctx, conn, func(tx *sql.Tx) error {
				if err := p.store.CreateVersionTable(ctx, tx); err != nil {
					return err
				}
				return p.store.Insert(ctx, tx, database.InsertRequest{Version: sentinel})
			}); err != nil {
...
// isSentinelVersion reports whether v is the sentinel version for this provider.
func (p *Provider) isSentinelVersion(v int64) bool {
