...
func TestMySQLConcurrentTableLocking(t *testing.T) {
	t.Parallel()
	if testing.Short() {
...
func TestMySQLSequentialTableLocking(t *testing.T) {
	t.Parallel()
	if testing.Short() {
		t.Skip("skipping test in short mode")
	}
	...
	locker1, err := lock.NewMySQLTableLocker(
	...
	locker2, err := lock.NewMySQLTableLocker(
		lock.WithTableLockID(lockID),
...
func TestMySQLLockerImplementations(t *testing.T) {
	t.Parallel()
	if testing.Short() {
		t.Skip("skipping integration test in short mode")
	}

	t.Run("mysql_table_locker_unique", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("mysql_table_locker_shared", func(t *testing.T) {
		t.Parallel()
...
func TestMySQLTableLockerIntegration(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping integration test in short mode")
	}
	db, cleanup, err := testdb.NewMariaDB()
	...
	t.Run("cleanup_stale_locks", func(t *testing.T) {
		...
		locker, err := lock.NewMySQLTableLocker(
			lock.WithTableLockID(lockID),
			lock.WithTableLeaseDuration(1*time.Second), // Short lease
			lock.WithTableHeartbeatInterval(200*time.Millisecond),
		)
		require.NoError(t, err)
